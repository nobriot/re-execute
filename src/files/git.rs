...
// readability. A line starting with # serves as a comment. Put a backslash
...
// literal "!", for example, "\!important!.txt". The slash "/" is used as the
// directory separator. Separators may occur at the beginning, middle or end of
// the .gitignore search pattern. If there is a separator at the beginning or
// middle (or both) of the pattern, then the pattern is relative to the
// directory level of the particular .gitignore file itself. Otherwise the
// pattern may also match at any level below the .gitignore level. If there is a
...
// directory (all paths are relative from the .gitignore file). An asterisk "*"
// matches anything except a slash. The character "?" matches any one character
// except "/". The range notation, e.g. [a-zA-Z], can be used to match one of
// the characters in a range. See fnmatch(3) and the FNM_PATHNAME flag for a
// more detailed description. Two consecutive asterisks ("**") in patterns
// matched against full pathname may have special meaning: A leading "**"
...
// directory "foo". A trailing "/**" matches everything inside. For example,
// "abc/**" matches all files inside directory "abc", relative to the location
// of the .gitignore file, with infinite depth. A slash followed by two
// consecutive asterisks then a slash matches zero or more directories. For
// example, "a/**/b" matches "a/b", "a/x/b", "a/x/y/b" and so on.
...
struct GitIgnoreRule {
    ...
    is_negated: bool,
    /// Regular matching, from the root of the .gitignore file, or at any dir
    /// level in between
...
impl GitIgnoreRule {
    ...
    fn string_matches(&self, file: &str, rule: &[GitIgnoreRuleElements]) -> bool {
        ...
        if let Some(GitIgnoreRuleElements::Slash) = rule_elements.peek() {
            ...
            // If we just pop'ed a slash, but the string also happens to be prepended with a
            // slash, remove it also
...
impl GitIgnoreRules {
    ...
    /// Starts collecting GitIgnoreRules from the path, going up to the watch
    /// directory
